    ...
    public partial class MainWindow : System.Windows.Window
    {
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            using (FileStream stream = new FileStream(file.FullName, FileMode.Open, FileAccess.Read))
            {
                ...
                socket.Receive(buffer, 4, SocketFlags.None); // byte로 받음
                int fileSize = BitConverter.ToInt32(buffer, 0); // int 형변환
                byte[] content = new byte[fileSize]; // 이미지 사이즈만큼 확보
                int l = socket.Receive(content, fileSize, SocketFlags.None); // 데이터 받기
                wFile.Write(content, 0, l); // 파일을 그려준다.

                //while(l != fileSize)
                //{
                //    byte[] Temp = new byte[10000]; // 1. 저장할공간
                //    int T = socket.Receive(Temp, 10000, SocketFlags.None); // 2.데이터받아서
                //    ssassageBox.Show(T.ToString());
                //    l += T; // 3. 1.공간에 더해준다.
                //    MessageBox.Show(l.ToString());
                //    wFile.Write(Temp, 0, T); // 받은 데이터를 추가로 그려준다.
                //    // fileSize만큼의 크기가 되면 while 빠져나간다. == 데이터를 다 받았다?
                //}
